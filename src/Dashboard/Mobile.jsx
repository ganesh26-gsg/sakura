...
const Mobiles = () => {
  ...
  useEffect(() => {
    axios
      ...
      .then((response) => {
        setMobiles(response.data.products.slice(0, 4)); // First 4 mobiles
      })
      .catch((error) => {
  ...
  return (
    <div className="p-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
        {mobiles.map((mobile) => (
          <div key={mobile.id} className="group relative bg-[#f5f5f5] p-6 rounded-lg overflow-hidden shadow-lg">
            <Link to="/mobile">
              <img
                src={mobile.images[0]}
                alt={mobile.title}
