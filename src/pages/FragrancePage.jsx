      ...
      <div className="flex pagePadding">
        ...
        <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-6 p-4">
          {filtered.map((item) => (
            ...
            >
              <Link to={`/fragrances/${item.id}`} className="block relative">
                <img
                  src={item.images[0]}
                  alt={item.title}
                  className="w-full h-[500px] object-cover rounded transition-transform duration-300 group-hover:scale-105"
                />
                <button
