  ...
  useEffect(() => {
    axios.get('https://dummyjson.com/products/category/womens-shoes')
      .then((res) => {
        ...
        setFilteredShoes(res.data.products);
      })
      .catch((err) => {
      ...
      <div className="flex pagePadding">
            ...
            <div>
              ...
              {brandOpen && (
                <div className="mt-3 space-y-2">
                  {shoes.map((shoe) => (
                    <label key={shoe.id} className="flex items-center gap-2">
                      <input
                        type="checkbox"
                        checked={selectedTitles.includes(shoe.title)}
                        onChange={() => handleCheckboxChange(shoe.title)}
        ...
        <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-5 gap-6 p-4">
          {filteredShoes.map((shoe) => (
            ...
            >
              <Link to={`/womenshoes/${shoe.id}`} className="block relative">
                <img
                  src={shoe.images[0]}
                  alt={shoe.title}
              ...
              <div className="transition-opacity duration-300 group-hover:opacity-0">
                <h1 className="text-sm font-medium mt-2">{shoe.title}</h1>
                <p className="text-gray-700 font-semibold">
                  ₹{(shoe.price * exchangeRate).toFixed(2)}
                </p>
              </div>
            </div>
          ))}
